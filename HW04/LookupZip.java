...
public class LookupZip {
    ...
    public static Place lookupZip(ExpandableArray<Place> places, String zip) {
        ...
        int index = binarySearch(places, zip);
        if (index != -1) {
    ...
    private static int binarySearch(ExpandableArray<Place> places, String zip) {
        int low = 0;
        int high = places.size() - 1;

        while (low <= high) {
            int mid = (low + high) / 2;
            Place place = places.get(mid);
            int cmp = place.getZip().compareTo(zip);

            if (cmp == 0) {
                return mid; // Zip code found
            } else if (cmp > 0) {
                low = mid + 1; 
            } else {
                high = mid - 1; 
